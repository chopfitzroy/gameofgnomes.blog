...
-- After flattening the site, every indexed page is a note: the
-- `[index]` table already excludes the home page, so we just accept
-- every entry soupault hands us.
function in_section(entry)
  -- Soupault embeds lua-ml, which predates Lua's boolean type: the only
  -- falsy value is nil. Returning a non-nil value (here, the number 1)
  -- is the idiomatic "true" in this dialect.
  return 1
end
